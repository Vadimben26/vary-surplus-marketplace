...
const REASON_KEYS = ["notReceived", "notMatching", "damaged", "wrongQuantity", "other"] as const;
...
export const DisputeForm = ({ order, buyerProfileId, buyerUserId, onCancel, onSubmitted }: Props) => {
  ...
  const addFiles = (newFiles: FileList | null) => {
    ...
    Array.from(newFiles).forEach((f) => {
      ...
      if (f.size > MAX_SIZE) {
        toast.error(t("disputeForm.fileTooLarge", { name: f.name }));
        return;
      }
      const ok = f.type.startsWith("image/") || f.type === "application/pdf";
      if (!ok) {
        toast.error(t("disputeForm.fileUnsupported", { name: f.name }));
        return;
      }
      added.push(f);
    });
  ...
  const canSubmit = !!reasonKey && details.trim().length >= 50 && !submitting;
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Persist the canonical (untranslated) reason label so admin/back-office
      // remains consistent regardless of the buyer's UI language.
      const reason = t(`disputeForm.reasons.${reasonKey}`, { lng: "fr" });
  ...
  return (
    <div className="mt-4 p-4 rounded-xl border border-amber-300 bg-amber-50/40 space-y-3">
      ...
      <div>
        <label className="text-xs font-semibold text-foreground block mb-1">{t("disputeForm.reasonLabel")}</label>
        <select
          value={reasonKey}
        ...
        >
          <option value="">{t("disputeForm.selectPlaceholder")}</option>
          {REASON_KEYS.map((key) => (
            <option key={key} value={key}>{t(`disputeForm.reasons.${key}`)}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        <label className="text-xs font-semibold text-foreground block mb-1">
          {t("disputeForm.evidenceLabel", { max: MAX_FILES })}
        </label>
